 ...
 *
 */
public class SetFileReNameTo extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					SetFileReNameTo frame = new SetFileReNameTo();
...
//				table.setModel(new DefaultTableModel(objects, new String[] { "A", "B" }));
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// 得到模板字符串
				StringBuffer template = new StringBuffer(panel_1.getText());

				// 得到第一次出现的位置
				int firstChar = template.toString().indexOf('#');

				// 第一次不出现’#‘的位子
				int firstNoChar = fileBiz.getIndexNoChar(template, '#');
				// 去除掉#的字符串
				String str = fileBiz.deleteCharToString(template, '#');
				System.out.println(str);
				// 得到#的个数
				int charSize = fileBiz.getFindChar(template.toString(), '#');

				// 获取到JSpinner的值
				Integer size = (Integer) spinner.getValue();

				// 再获得后缀名
				String suffix = panel_2.getText();

				File[] files = fileBiz.getAllFile(panel.getText());

				// 建立一个可存放原名称 和 修改后的名称的object二维数组
				objects = new Object[files.length][2];
				for (int i = size; i < files.length; i++) {

					objects[i - size][0] = new String(files[i].getName());
					...
					// 再拼接上重命名的名称
					String reName = "";
					if (firstChar < firstNoChar) {
						reName = ((int)Math.pow(10, charSize - 1) + i - size)+ str + "." + suffix;
					} else {
						reName = (str + ((int)Math.pow(10, charSize - 1) + i - size) + "." + suffix);
					}
					...
					files[i].renameTo(new File(path.toString()));
					// 然后添加到objects的二维数组中
					objects[i - size][1] = new String(reName);
				}
				table.setModel(new DefaultTableModel(objects, new String[] { "A", "B" }));
			}
